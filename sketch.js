...
const myMainCanvasSketch = (p) => {
  ...
  const SWIPE_RIGHT_Y =
    SWIPE_LEFT_Y + 18 + 9 + CARD_H + 9 + 18;

  const CARD_Y =
    (SWIPE_LEFT_Y + SWIPE_RIGHT_Y) / 2;
  ...
  p.setup = async () => {
    ...
    const currentWidth =
      container ? container.clientWidth : 960;
  ...
  p.draw = () => {
    ...
    /*
     * Middle event panel intentionally removed.
     */
  ...
  p.mouseWheel = (event) => {
    const mx =
      p.mouseX * (DESIGN_W / p.width);

    const my =
      p.mouseY * (DESIGN_H / p.height);
  ...
  function startNewGame() {
    ...
    drawPile =
      p.shuffle([...Array(cardsData.length).keys()]);
  ...
  function queueAchievement(id, label) {
    ...
    const index =
      currentAchievementList.findIndex(
        a => a.id === id
      );

    if (index !== -1) {
      const [achItem] =
        currentAchievementList.splice(
          index,
          1
        );

      currentAchievementList.unshift(
        achItem
      );
    }
  ...
  p.mousePressed = () => {
    ...
    if (gameOver) {
      if (
        ...
        /*
         * Start the virus exactly
         * 3 seconds after the new
         * game begins.
         */
        startVirusAfterRestart();
      }
...
window.virusTriggerTimeout = null;

const CORRUPTION_DURATION_MS = 10000;
const AD_SPAWN_INTERVAL_MS = 350;
const VIRUS_RESTART_DELAY_MS = 3000;
...
/* ==========================================================================
   START VIRUS 3 SECONDS AFTER PLAY AGAIN
   ========================================================================== */

function startVirusAfterRestart() {
  if (window.virusTriggerTimeout) {
    clearTimeout(
      window.virusTriggerTimeout
    );
  }
...
window.triggerGlobalGlitch =
  function triggerCorruption() {
    ...
    window.adSpamInterval =
      setInterval(
        spawnSpamAd,
        AD_SPAWN_INTERVAL_MS
      );

    window.glitchTimeoutId =
      setTimeout(() => {
        if (
          window.adSpamInterval
        ) {
